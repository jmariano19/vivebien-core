...
// ASSET LOADING
// ──────────────────────────────────────────────
function loadBase64Asset(filename) {
    ...
    logger_1.logger.warn({ filename }, 'Asset not found, PDF will render without it');
    return '';
}
// ──────────────────────────────────────────────
// HELPERS
// ──────────────────────────────────────────────
function autoBoldBullet(text) {
    ...
    if (prefix.includes('<b>') && prefix.includes('</b>'))
        return text;
...
function mapToPdfData(raw) {
    ...
    const signalItems = (data.signal_items || []).map((item) => ({
...
// PDF GENERATION
// ──────────────────────────────────────────────
async function generateSummaryPdf(summaryData) {
    ...
    try {
        ...
        const pageHeight = await page.evaluate('document.querySelector(".page")?.scrollHeight || 800');
        ...
        await browser.close();
        logger_1.logger.info({ size: pdfBuffer.length, pageHeight }, 'PDF generated successfully');

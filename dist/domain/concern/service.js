...
class ConcernService {
     ...
     */
    async getActiveConcerns(userId) {
        const result = await this.db.query(`SELECT id, user_id, title, status, summary_content, icon, created_at, updated_at
     ...
     */
    async getConcernById(concernId, userId) {
        const query = userId
     ...
     */
    async getOrCreateConcern(userId, title, icon) {
        ...
        const matchedTitle = (0, matching_1.findBestConcernMatch)(title, existingTitles);
        if (matchedTitle) {
     ...
     */
    isSymptomOfCondition(condition, symptom) {
        const conditionSymptomMap = {
            // Flu / Cold / COVID
     ...
     */
    async renameConcern(concernId, newTitle) {
        await this.db.query(`UPDATE health_concerns SET title = $1, updated_at = NOW() WHERE id = $2`, [newTitle, concernId]);
        logger_1.logger.info({ concernId, newTitle }, 'Concern renamed (language change)');
    }
    /**
     * Create a brand new concern
     */
    async createConcern(userId, title, icon) {
        const result = await this.db.query(`INSERT INTO health_concerns (id, user_id, title, status, icon, created_at, updated_at)
       VALUES (gen_random_uuid(), $1, $2, 'active', $3, NOW(), NOW())
       RETURNING id, user_id, title, status, summary_content, icon, created_at, updated_at`, [userId, title, icon || null]);
        const concern = this.mapRow(result.rows[0]);
        logger_1.logger.info({ userId, concernId: concern.id, title }, 'New health concern created');
        return concern;
    }
    /**
     * Update a concern's summary. Creates a snapshot if the change is meaningful.
     */
    async updateConcernSummary(concernId, newContent, changeType) {
        ...
        try {
            ...
            // Update the concern
            await client.query(`UPDATE health_concerns
         SET summary_content = $1, updated_at = NOW()
         WHERE id = $2`, [newContent, concernId]);
            // Create snapshot if meaningful change
            if (shouldSnapshot) {
                await client.query(`INSERT INTO concern_snapshots (id, concern_id, user_id, content, change_type, status, created_at)
           VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, NOW())`, [concernId, concern.userId, newContent, changeType, concern.status]);
                logger_1.logger.info({ concernId, changeType, userId: concern.userId }, 'Concern snapshot created');
            }
     ...
     */
    async updateConcernStatus(concernId, newStatus) {
        const concern = await this.getConcernById(concernId);
        if (!concern) {
            throw new Error(`Concern not found: ${concernId}`);
        }
        ...
        try {
         ...
         WHERE id = $2`, [newStatus, concernId]);
            // Always snapshot status changes
            await client.query(`INSERT INTO concern_snapshots (id, concern_id, user_id, content, change_type, status, created_at)
         VALUES (gen_random_uuid(), $1, $2, $3, 'status_change', $4, NOW())`, [concernId, concern.userId, concern.summaryContent || '', newStatus]);
            await client.query('COMMIT');
            logger_1.logger.info({ concernId, oldStatus: concern.status, newStatus, userId: concern.userId }, 'Concern status updated');
        }
        catch (error) {
            await client.query('ROLLBACK');
            throw error;
     ...
     */
    async deleteConcern(concernId) {
        await this.db.query(`DELETE FROM health_concerns WHERE id = $1`, [concernId]);
        logger_1.logger.info({ concernId }, 'Concern deleted');
    }
    /**
     * Get the full snapshot history for a concern
     */
    async getConcernHistory(concernId) {
        const result = await this.db.query(`SELECT id, concern_id, user_id, content, change_type, status, created_at
       FROM concern_snapshots
       WHERE concern_id = $1
       ORDER BY created_at DESC`, [concernId]);
     ...
     */
    async getPrimaryConcern(userId) {
     ...
     */
    async getRecentUserEdits(userId, since) {
        const result = await this.db.query(`SELECT hc.title, cs.content, cs.created_at
     ...
     */
    hasMeaningfulChange(oldContent, newContent) {
        if (!oldContent)
            return true;

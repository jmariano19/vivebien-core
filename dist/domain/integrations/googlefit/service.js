...
exports.googleFitService = exports.GoogleFitService = void 0;
...
class GoogleFitService {
    ...
    async isConnected(userId) {
        const result = await client_1.db.query('SELECT id FROM google_fit_tokens WHERE user_id = $1', [userId]);
        return result.rows.length > 0;
    }
    async disconnect(userId) {
        await client_1.db.query('DELETE FROM google_fit_tokens WHERE user_id = $1', [userId]);
    }
    // ── Data Fetching ──────────────────────────────────────────────────────────
    async fetchTodayData(userId) {
        ...
        const parts = ['[Google Fit]'];
        if (sleepHours)
            parts.push(`Sleep: ${sleepHours}h`);
        if (steps)
            parts.push(`Steps: ${steps.toLocaleString()}`);
        if (restingHeartRate)
            parts.push(`Resting HR: ${restingHeartRate} bpm`);
        if (activity?.activeMinutes)
            parts.push(`Active: ${activity.activeMinutes} min`);
        if (activity?.calories)
            parts.push(`Calories: ${activity.calories} kcal`);
        return {
            sleepHours,
            steps,
            restingHeartRate,
    ...
    async fetchSleepHours(accessToken, startMs, endMs) {
    ...
    async fetchSteps(accessToken, startMs, endMs) {
        const res = await googleFitPost(accessToken, '/dataset:aggregate', {
    ...
    async fetchRestingHR(accessToken, startMs, endMs) {
        const res = await googleFitPost(accessToken, '/dataset:aggregate', {
    ...
    async fetchActivity(accessToken, startMs, endMs) {
        const res = await googleFitPost(accessToken, '/dataset:aggregate', {
            aggregateBy: [
    ...
    async getValidTokens(userId) {
        const result = await client_1.db.query('SELECT access_token, refresh_token, expires_at FROM google_fit_tokens WHERE user_id = $1', [userId]);
    ...
    async storeTokens(userId, tokens) {
        await client_1.db.query(`INSERT INTO google_fit_tokens (user_id, access_token, refresh_token, expires_at)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id) DO UPDATE
       SET access_token = $2, refresh_token = $3, expires_at = $4, updated_at = NOW()`, [userId, tokens.accessToken, tokens.refreshToken, tokens.expiresAt]);
    }
}
exports.GoogleFitService = GoogleFitService;
// ─── Helpers ──────────────────────────────────────────────────────────────────
async function googleFitPost(accessToken, path, body) {
...
exports.googleFitService = new GoogleFitService();

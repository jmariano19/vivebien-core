...
exports.ARCHETYPE_FIRST_IMPRESSION = exports.ONBOARDING_COMPLETE = exports.ONBOARDING_INTRO = exports.ONBOARDING_QUESTIONS = void 0;
exports.detectArchetype = detectArchetype;
exports.getQuestion = getQuestion;
exports.getOnboardingIntro = getOnboardingIntro;
exports.getOnboardingComplete = getOnboardingComplete;
exports.getArchetypeMessage = getArchetypeMessage;
// ============================================================================
...
exports.ARCHETYPE_FIRST_IMPRESSION = {
    performance: {
 ...
 */
function scoreAnswer(questionIndex, answer) {
    const lower = answer.toLowerCase();
    const scores = { performance: 0, skeptic: 0, curious: 0, passive: 0 };
    if (questionIndex === 0) {
        // Q1: What brought you here / what do you want to understand
        if (/energía|energy|rendimiento|performance|gym|entreno|workout|músculo|muscle|optimizar|optimize/.test(lower))
            scores.performance += 2;
        if (/por qué|why|cómo funciona|how does|mecanismo|mechanism|entender|understand|curiosi/.test(lower))
            scores.curious += 2;
        if (/intenté|tried|fallé|failed|no me funcionó|didn't work|escéptico|skeptic|duda|doubt/.test(lower))
            scores.skeptic += 2;
        if (/no sé|don't know|alguien me dijo|someone told me|me mandaron|they sent me|no tengo idea/.test(lower))
            scores.passive += 2;
    }
    if (questionIndex === 1) {
        // Q2: Relationship with food right now
        if (/control|disciplina|discipline|estructura|structure|seguimiento|tracking|mido|measure/.test(lower))
            scores.performance += 2;
        if (/complicada|complicated|difícil|difficult|confusa|confused|no entiendo|don't understand/.test(lower))
            scores.curious += 1;
        if (/mala|bad|culpa|guilty|vergüenza|shame|fallé|failed|siempre fallo|always fail/.test(lower))
            scores.skeptic += 2;
        if (/normal|igual|así nomás|okay i guess|no pienso|don't think about|como lo que hay|eat what's there/.test(lower))
            scores.passive += 2;
        if (/interesante|interesting|fascinante|fascinating|quiero aprender|want to learn/.test(lower))
            scores.curious += 2;
    }
    if (questionIndex === 2) {
        // Q3: Last time tried to change eating — what happened
        if (/funcionó|worked|logré|achieved|mejoró|improved|bien|well/.test(lower))
            scores.performance += 1;
        if (/no funcionó|didn't work|dejé|quit|rendí|gave up|difícil|hard|fracasé|failed/.test(lower))
            scores.skeptic += 2;
        if (/aprendí|learned|descubrí|discovered|interesante|interesting|me di cuenta|realized/.test(lower))
            scores.curious += 2;
        if (/nunca|never|no recuerdo|don't remember|hace mucho|long time ago|no he intentado|haven't tried/.test(lower))
            scores.passive += 2;
        if (/experimento|experiment|probé|tried|variante|variant|sistema|system/.test(lower))
            scores.performance += 2;
    }
    if (questionIndex === 3) {
        // Q4: Exercise frequency + how you feel after
        if (/todos los días|every day|diario|daily|gym|5 veces|6 veces|5 times|6 times|entreno regular|regular training/.test(lower))
            scores.performance += 2;
        if (/a veces|sometimes|cuando puedo|when i can|irregular|no mucho|not much/.test(lower))
            scores.passive += 1;
        if (/nunca|never|no hago|don't exercise|no puedo|can't/.test(lower))
            scores.passive += 2;
        if (/siento que|i feel like|noto que|i notice|me pregunto|i wonder/.test(lower))
            scores.curious += 1;
        if (/energía|energy|rendimiento|performance|recuperación|recovery|músculo|muscle/.test(lower))
            scores.performance += 1;
    }
    if (questionIndex === 4) {
        // Q5: Anything you want me to know before starting
        if (/nada|nothing|no|todo bien|all good/.test(lower) && answer.length < 30)
            scores.passive += 1;
        if (/privacidad|privacy|datos|data|cómo funciona|how does this work|seguro|safe/.test(lower))
            scores.skeptic += 2;
        if (/pregunta|question|duda|doubt|quiero saber|want to know|explícame|explain/.test(lower))
            scores.curious += 2;
        if (/objetivo|goal|meta|target|quiero lograr|want to achieve|plan/.test(lower))
            scores.performance += 2;
...
function detectArchetype(answers) {
    const total = { performance: 0, skeptic: 0, curious: 0, passive: 0 };
 ...
 */
function getQuestion(step, language) {
    const lang = language in exports.ONBOARDING_QUESTIONS ? language : 'es';
    const questions = exports.ONBOARDING_QUESTIONS[lang];
    return questions[step - 1] ?? questions[0];
}
/**
 * Get the intro message (sent before Q1) in the right language.
 */
function getOnboardingIntro(language) {
    const lang = language in exports.ONBOARDING_INTRO ? language : 'es';
    return exports.ONBOARDING_INTRO[lang];
}
/**
 * Get the completion message (sent after Q5, before archetype message) in the right language.
 */
function getOnboardingComplete(language) {
    const lang = language in exports.ONBOARDING_COMPLETE ? language : 'es';
    return exports.ONBOARDING_COMPLETE[lang];
}
/**
 * Get the archetype-specific first impression message.
 */
function getArchetypeMessage(archetype, language) {
    const lang = language in exports.ONBOARDING_COMPLETE ? language : 'es';
    return exports.ARCHETYPE_FIRST_IMPRESSION[archetype][lang];
}

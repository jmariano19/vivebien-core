...
const concernRoutes = async (app) => {
    ...
    app.get('/:userId', async (request, reply) => {
        ...
        const user = await (0, client_1.queryOne)(`SELECT id, COALESCE(language, 'es') as language, name FROM users WHERE id = $1`, [userId]);
        if (!user) {
    ...
    app.get('/:userId/:concernId', async (request, reply) => {
        const { userId, concernId } = request.params;
        ...
        try {
            ...
            const user = await (0, client_1.queryOne)(`SELECT COALESCE(language, 'es') as language, name FROM users WHERE id = $1`, [userId]);
            return {
                ...concern,
    ...
    app.put('/:userId/:concernId', async (request, reply) => {
        const { userId, concernId } = request.params;
        const { summary, title } = request.body;
        if (!UUID_REGEX.test(userId) || !UUID_REGEX.test(concernId)) {
            return reply.status(400).send({ error: 'Invalid ID format' });
        }
        const concern = await concernService.getConcernById(concernId);
        ...
        try {
            ...
            if (title) {
                await client_1.db.query(`UPDATE health_concerns SET title = $1, updated_at = NOW() WHERE id = $2`, [title, concernId]);
            }
...
exports.concernRoutes = concernRoutes;

...
export class VoiceService {
  ...
  async synthesizeSpeech(
    ...
    if (!targetVoiceId) {
      logger.warn('No voice ID configured for ElevenLabs');
      return null;
    }

    try {
      logger.info({ voiceId: targetVoiceId, textLength: text.length }, 'Starting voice synthesis');
      ...
      logger.info(
        { audioSize: audioBuffer.length, durationEstimate },

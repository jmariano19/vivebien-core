...
export interface ConcernCommand {
  ...
  targets: string[]; // concern name(s) the user mentioned
 ...
 */
export function detectConcernCommand(message: string, language: string): ConcernCommand | null {
  ...
  if (mergeCommand) return mergeCommand;
  ...
  if (renameCommand) return renameCommand;
 ...
 */
export function getCommandConfirmationMessage(
 ...
 */
export function getCommandErrorMessage(language: string): string {
  switch (language) {
    case 'es':
